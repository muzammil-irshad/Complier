...
void LexicalAnalyzer::printTokens() {
	...
	for (int i = 0; i < tokens.size(); i++) {
		if (tokens[i]->type == "id")
			addEntity(tokens[i]->lexeme);
		fout << tokens[i]->type << '\t' << tokens[i]->lexeme;
		if (i != tokens.size() - 1)
			fout << endl;
	}
	fout.close();
...
void LexicalAnalyzer::addEntity(string str) {
	for (int i = 0; i < symbolTable.size(); ++i) {
		if (str == symbolTable[i]->id)
			return;
	}
	Entity *entity = new Entity();
	entity->id = str;
	symbolTable.push_back(entity);
